...
## Reg Owner Conflicts:
      ...
      WITH ownership_t AS (select
        ...
        and t1.created_date >= '01-JAN-2020 00:00:00'
        ORDER BY t3.vessel_conflict_id)
        SELECT RESOLVED_MONTH, RESOLVED_YEAR, CONFLICT_STATUS, DATA_FEED, COUNT(CONFLICT_STATUS) AS COUNT_STATUS_PER_FEED
        FROM ownership_t
        GROUP BY RESOLVED_MONTH, RESOLVED_YEAR, CONFLICT_STATUS, DATA_FEED
      ...
      WITH ownership_t AS (select
        ...
        and t1.created_date >= '01-JAN-2020 00:00:00'
        ORDER BY t3.vessel_conflict_id)
        SELECT RESOLVED_MONTH, RESOLVED_YEAR, DATA_FEED, COUNT(DATA_FEED) AS NUM_IGNORED
        FROM ownership_t
        WHERE CONFLICT_STATUS = 'IGNORED'
        GROUP BY RESOLVED_MONTH, RESOLVED_YEAR, DATA_FEED
        ORDER BY NUM_IGNORED DESC;


      -- Count conflicts by analyst, grouping by month, year, analyst, conflict status  
      WITH ownership_t AS (select
        t1.id
        , t3.file_data_line_id
        , t3.vessel_conflict_id
      ...
      WITH ownership_t AS (select
        ...
        and t1.created_date >= '01-JAN-2020 00:00:00'
        ORDER BY t3.vessel_conflict_id)
        SELECT RESOLVED_MONTH, RESOLVED_YEAR, DATA_FEED, COUNT(DATA_FEED) AS NUM_LIKENESS
        FROM ownership_t
        WHERE IGNORE_REASON = 'Likeness'
        GROUP BY RESOLVED_MONTH, RESOLVED_YEAR, DATA_FEED
        ORDER BY NUM_LIKENESS DESC;
