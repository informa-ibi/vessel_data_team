...
if __name__ == '__main__':
    ...
    output_lr_apply_df = lr_to_apply_df.merge(to_map_df, how='left',
                                              right_on=['ENGINE_DESIGNATION_lr',
                                                        'CYLINDERS_ARRANGEMENT_lr',
                                                        'NO_OF_CYLINDERS_lr', 'BORE_lr', 'STROKE_lr',
                                                        'FUEL_TYPE_lr', 'DESIGNER_lr'],
                                              left_on=['ENGINE_DESIGNATION',
                                                       'CYLINDERS_ARRANGEMENT',
                                                       'NO_OF_CYLINDERS', 'BORE', 'STROKE', 'FUEL_TYPE', 'DESIGNER'])

    def prepare_output_df(row):
        if row.is_mapped == 'mapped':

        else:
            row['output_ENGINE_DESIGNATION'] = row['_ENGINE_DESIGNATION']

    test_df = output_lr_apply_df.copy()
    test_df.loc[test_df.is_mapped=='mapped', 'output_ENGINE_DESIGNATION'] =
